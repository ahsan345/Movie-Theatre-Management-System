...
class MainPage(Frame):
    """Main page when the GUI is ran. Allows the user to select Manager, customer or staff"""
    ...
    def __init__(self, master):
        ...
        self.title_label = Label(self.master, font=("Helvetica", 15),
                                 text="Welcome to Movie Theatre Management System!! Please select your role"
                                      " to login.").pack(pady='3')
        self.man_button = Button(self.master, font=("Helvetica", 16), text="Manager", bg="grey",
                                 command=self.manager_window)
        self.man_button.pack(pady='3')
        self.man_button.bind("<Enter>", lambda event, b=self.man_button: b.config(bg="lightgrey"))
        self.man_button.bind("<Leave>", lambda event, b=self.man_button: b.config(bg="grey"))
        
        self.staff_button = Button(self.master, font=("Helvetica", 16), text="Staff", bg="grey",
                                 command=self.staff_window)
        self.staff_button.pack(pady='3')
        self.staff_button.bind("<Enter>", lambda event, b=self.staff_button: b.config(bg="lightgrey"))
        self.staff_button.bind("<Leave>", lambda event, b=self.staff_button: b.config(bg="grey"))

        self.cust_button = Button(self.master, font=("Helvetica", 16), text="Customer", bg="grey",
                                  command=self.customer_window)
        self.cust_button.pack(pady='3')
        self.cust_button.bind("<Enter>", lambda event, b=self.cust_button: b.config(bg="lightgrey"))
        self.cust_button.bind("<Leave>", lambda event, b=self.cust_button: b.config(bg="grey"))

        self.quit = Button(self.master, font=("Helvetica", 12), text="Quit", bg="red", fg='white',
    ...
    def manager_window(self):
        ...
        self.app = Manager(self.newWindow)
...
class Customer(MainPage):
    ...
    def __init__(self, master):
        ...
        # Login button with hover function
        self.loginbutton = Button(self.master, text="Login", command=self.login)
        self.loginbutton.grid(row=4, column=1, pady='3')
        self.loginbutton.bind("<Enter>", lambda event, btn=self.loginbutton: btn.config(bg="lightblue"))
        self.loginbutton.bind("<Leave>", lambda event, btn=self.loginbutton: btn.config(bg="SystemButtonFace"))
        
        # Back button with hover function
        self.back = Button(self.master, text="Back", command=self.back)
        self.back.grid(row=4, column=0, pady='3')
        self.back.bind("<Enter>", lambda event, btn=self.back: btn.config(bg="lightblue"))
        self.back.bind("<Leave>", lambda event, btn=self.back: btn.config(bg="SystemButtonFace"))
        
        # Create New Account button with hover function
        self.createnewaccount = Button(self.master, text="Create New Account", command=self.create)
        self.createnewaccount.grid(row=5, column=0, columnspan=2, pady='3')
        self.createnewaccount.bind("<Enter>", lambda event, btn=self.createnewaccount: btn.config(bg="lightblue"))
        self.createnewaccount.bind("<Leave>", lambda event, btn=self.createnewaccount: btn.config(bg="SystemButtonFace"))
...
class CustMain(Customer):
    '''Customer main menu page. Allows user to update profile, Make a ticket booking, View Booking Details, and Check ticket availability & Seat Selection'''
    ...
    def __init__(self, master):
        ...
        self.label.place(x=0, y=0, relwidth=1, relheight=1)
        self.frame.pack()
        self.up = Button(self.master, font=("Helvetica", 16), text="Update Profile", bg="lightblue",
                         command=self.Update_Profile_window).pack(pady='5')
        self.bh = Button(self.master, font=("Helvetica", 16), text="View Booking Details", bg="lightblue",
                         command=self.Booking_History_window).pack(pady='5')
        self.search = Label(self.master, font=("Helvetica", 16), text='Make a ticket Booking', bg="lightblue").pack(pady='5')
        self.defdate1 = StringVar(self.master)
        
        self.search = Label(self.master, font=("Helvetica", 16), text='Check ticket availability & Seat Selection', bg="lightblue").pack(pady='5')
        self.defdate1 = StringVar(self.master)
        self.defdate1.set("Monday 01/08/23")  # default value
        self.datelist1 = OptionMenu(master, self.defdate1, "Monday 14/08/23", "Tuesday 15/08/23", "Wednesday 16/08/23",
                                    "Thursday 17/08/23", "Friday 18/08/23", "Saturday 19/08/23", "Sunday 20/08/23")
        self.datelist1.pack()
        self.searching = Button(self.master, text="Check", command=self.sch, font=("Helvetica", 12), bg="lightblue").pack(pady='5')
        self.log = Button(self.master, font=("Helvetica", 12), text="Logout", fg="white", bg='red',
...
class SearchResults(CustMain):
    def __init__(self, master, output, search_date):
        ...
        self.back = Button(self.master, font=("Helvetica", 12), text="Back", bg="lightblue", fg="black",
    ...
    def boo(self, gone, datetime7):
        ...
        if msg:
            ...
            if alr:
                ...
                if td_year > 2023 or \
                        td_year == 2023 and td_month > 8 or \
                        td_year == 2023 and td_month == 8 and td_day > new_date or \
                        td_year == 2023 and td_month == 8 and td_day == new_date and td_hour >= new_time:
...
class BookHist(Booked):
    def __init__(self, master):
        ...
        c3.execute('''SELECT date, time, seatno FROM bookings WHERE
                    username = ? ORDER BY date='Monday 14/08/23' DESC,
                                                    date='Tuesday 15/08/23' DESC,
                                                    date='Wednesday 16/08/23' DESC,
                                                    date='Thursday 17/08/23' DESC,
                                                    date='Friday 18/08/23' DESC,
                                                    date='Saturday 19/08/23' DESC,
                                                    date='Sunday 20/08/23' DESC,
                                                    time='1pm' DESC,
                                                    time='2pm' DESC,
                                                    time='3pm' DESC,
                                                    time='4pm' DESC,
...
class Manager(Customer):
    ...
    def login(self):
        ...
        lines = iter([line.rstrip('\n') for line in open('manager.txt', 'r')])
        for i in lines:
            j = i.split(': ')
...
class ManagerMain(Manager):
    '''Manager main menu page. Allows Manager to Add Movie, Schedule Shows and Add a customer as a member'''
    ...
    def __init__(self, master):
        ...
        self.label.place(x=0, y=0, relwidth=1, relheight=1)
        self.frame.pack()
        self.add_movie = Button(self.master, font=("Helvetica", 16), text="Add Movie", bg="lightgreen",
                               command=self.add_movie_func)
        self.add_movie.pack(pady='5')
        self.add = Button(self.master, font=("Helvetica", 16), text="Schedule Shows", bg="lightgreen",
                          command=self.Add_film_showings)
        self.add.pack(pady='5')
        self.add_member = Button(self.master, font=("Helvetica", 16), text="Add a Customer as a Member", bg="lightgreen",
                          command=self.Add_member)
        self.add_member.pack(pady='5')
        self.log = Button(self.master, font=("Helvetica", 12), text="Logout", fg="white", bg='red',
                          command=self.logout)
        self.log.pack(pady='5')

    def add_movie_func(self):
        # create a new window for adding a movie
        add_movie_window = Toplevel(self.master)
        add_movie_window.title("Add Movie")
        add_movie_window.geometry("400x400")

        # create labels and entry fields for movie details
        title_label = Label(add_movie_window, text="Name:")
        title_label.pack()
        title_entry = Entry(add_movie_window)
        title_entry.pack()

        duration_label = Label(add_movie_window, text="Duration (mins):")
        duration_label.pack()
        duration_entry = Entry(add_movie_window)
        duration_entry.pack()

        languages_label = Label(add_movie_window, text="Available Languages:")
        languages_label.pack()
        languages_entry = Entry(add_movie_window)
        languages_entry.pack()

        types_label = Label(add_movie_window, text="Available Types (2D, 3D, 4D):")
        types_label.pack()
        types_entry = Entry(add_movie_window)
        types_entry.pack()

        last_date_label = Label(add_movie_window, text="Last Date (yyyy-mm-dd):")
        last_date_label.pack()
        last_date_entry = Entry(add_movie_window)
        last_date_entry.pack()

        # create a button to submit movie details
        submit_button = Button(add_movie_window, text="Submit", command=lambda: self.add_movie_to_database(title_entry.get(), duration_entry.get(), languages_entry.get(), types_entry.get(), last_date_entry.get()), bg="lightgrey", activebackground="blue", cursor="hand2")
        submit_button.pack()

        # add a hover effect to the button
        def on_enter(e):
            submit_button['background'] = 'lightblue'

        def on_leave(e):
            submit_button['background'] = 'lightgrey'
    ...
    def add_movie_to_database(self, title, duration, languages, types, last_date):
        # validate and parse movie details
        if not all([title, duration, languages, types, last_date]):
            messagebox.showerror("Error", "Please enter all movie details.")
            return
        ...
        try:
            ...
            messagebox.showerror("Error", "Invalid duration or date format.")
            return
        ...
        c = conn.cursor()
        c.execute("INSERT INTO movies (date, time, title, description, booked, available) VALUES (?, ?, ?, ?, ?, ?)", (last_date, '00:00', title, languages + ', ' + ', '.join(types), 0, 1))
        conn.commit()
        conn.close()
    ...
    def Add_member(self):
        ...
        self.newWindow.title('Add Customer as a Member')

        # Create the labels and entry widgets for member type, deposit amount, and member code
        Label(self.newWindow, text="Membership Type:").grid(row=0, column=0, padx=10, pady=10)
        member_type = StringVar(self.newWindow)
        member_type.set("Select") # default value
        options = ["Gold Member", "Silver Member", "Bronze Member"]
        OptionMenu(self.newWindow, member_type, *options).grid(row=0, column=1, padx=10, pady=10)

        Label(self.newWindow, text="Deposit Amount (AED):").grid(row=1, column=0, padx=10, pady=10)
        deposit_entry = Entry(self.newWindow)
        deposit_entry.grid(row=1, column=1, padx=10, pady=10)

        Label(self.newWindow, text="Member Code:").grid(row=2, column=0, padx=10, pady=10)
        code_entry = Entry(self.newWindow)
        code_entry.grid(row=2, column=1, padx=10, pady=10)

        # Create the button to add the member
        add_button = Button(self.newWindow, text="Add Member", command=lambda: self.add_member(member_type, deposit_entry, code_entry))
        add_button.grid(row=3, column=0, columnspan=2, padx=10, pady=10)

    def add_member(self, member_type, deposit_entry, code_entry):
        member_type_val = member_type.get()
        deposit_val = deposit_entry.get()
        code_val = code_entry.get()

        if member_type_val == "Select":
            tkinter.messagebox.showerror("Error", "Please select a membership type.")
        elif not deposit_val:
            tkinter.messagebox.showerror("Error", "Please enter the deposit amount.")
        elif not code_val:
            tkinter.messagebox.showerror("Error", "Please enter a code for the membership.")
        else:
            try:
                deposit_val = float(deposit_val)
            except ValueError:
                tkinter.messagebox.showerror("Error", "Deposit amount must be a number.")
                return

            if deposit_val < 200:
                tkinter.messagebox.showerror("Error", "Deposit amount must be at least 200 AED.")
                return

            if member_type_val == "Gold Member":
                discount_self = 0.5
                discount_others = 0.5
            elif member_type_val == "Silver Member":
                discount_self = 0.25
                discount_others = 0.1
            else:
                discount_self = 0.15
                discount_others = 0.05

            # Save the member data to the database or do other necessary processing here

            tkinter.messagebox.showinfo("Success", "Member added successfully.")

class Showings(ManagerMain):
    '''Shows a list of times and dates that have a film showing. The dates range from Monday 14th - Friday 20th
    August 2023, and times range from 1-11pm (with the last showing at 10pm). The shows are for exactly
    one hour'''
    ...
    def __init__(self, master):
        ...
        self.seeseats = Label(self.master, text='Seating', font=("Helvetica", 16), width=8).grid(row=0, column=7,
                                                                                                 pady='1')
        c2.execute('''SELECT * FROM movies ORDER BY date='Monday 14/08/23' DESC,
                                                    date='Tuesday 15/08/23' DESC,
                                                    date='Wednesday 16/08/23' DESC,
                                                    date='Thursday 17/08/23' DESC,
                                                    date='Friday 18/08/23' DESC,
    ...
    def export(self):
        try:
            ...
            c2.execute('''SELECT * FROM movies ORDER BY date='Tuesday 14/08/23' DESC,
                                                    date='Wednesday 15/08/23' DESC,
                                                    date='Thursday 16/08/23' DESC,
                                                    date='Friday 17/08/23' DESC,
                                                    date='Saturday 18/08/23' DESC,
                                                    date='Sunday 19/08/23' DESC,
                                                    date='Monday 20/08/23' DESC,
                                                    time='1pm' DESC,
                                                    time='2pm' DESC,
                                                    time='3pm' DESC,
                                                    time='4pm' DESC,
...
class AddShowings(Showings):
    def __init__(self, master):
        ...
        self.defdate.set("Tuesday 01/08/23")  # default value
        self.datelist = OptionMenu(master, self.defdate, "Tuesday 14/08/23", "Wednesday 15/08/23", "Thursday 16/08/23",
                                   "Friday 17/08/23", "Saturday 18/08/23", "Sunday 19/08/23", "Monday 20/08/23")
    ...
    def login(self):
        ...
        for i in lines:
            ...
            if username2 == j[0] and password2 == j[1]:
                self.newWindow = Toplevel(self.master)
                self.newWindow.geometry('1096x720')
                self.app = StaffMain(self.newWindow)
                self.master.withdraw()
                tkinter.messagebox.showinfo("---- SUCCESS ----", "You Have Now Logged In.", icon="info")
                break
        else:
...
class StaffMain(Staff):
    '''Staff main menu page. Allows user to add films and see films'''
    ...
    def __init__(self, master):
        ...
        self.add_button = Button(self.master, font=("Helvetica", 16), text="Make a ticket booking for Customer", bg="lightblue",
                          command=self.Add_film_showings, activebackground="lightblue", activeforeground="blue")
        self.add_button.pack(pady=5)

        self.showings_button = Button(self.master, font=("Helvetica", 16), text="View Booking Details", bg="lightblue",
                                    command=self.See_list, activebackground="lightblue", activeforeground="blue")
        self.showings_button.pack(pady=5)

        self.logout_button = Button(self.master, font=("Helvetica", 12), text="Logout", fg="white", bg='red',
                                command=self.logout, activebackground="red", activeforeground="black")
...
class AddShowings(Showings):
    def __init__(self, master):
        ...
        self.date = Label(self.master, text="Date:", width=15).grid(row=1, column=0, pady='3')
        self.defdate = StringVar(self.master)
        self.defdate.set("Tuesday 14/08/23")  # default value
        self.datelist = OptionMenu(master, self.defdate, "Tuesday 14/08/23", "Wednesday 15/08/23", "Thursday 16/08/23",
                                   "Friday 17/08/23", "Saturday 18/08/23", "Sunday 19/08/23", "Monday 20/08/23")
        # self.datelist.config(width=30)
        self.datelist.grid(row=1, column=1, pady='3')
...
def main():
    window = Tk()
    window.title("Movie Theatre Management System")
